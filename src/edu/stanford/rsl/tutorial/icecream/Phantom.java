...
public class Phantom extends Grid2D {
	...
	/*
	 * private void setDetectorSpacing(float ds) { this.detectorSpacing = ds; }
	 * 
	 * public float getDetectorSpacing() { return detectorSpacing; }
	 */
				...
				for (double t = -SID; t < SID; t = t + xspacePhantom) {
					...
					if (id[0] >= 0 && id[0] < this.getWidth()) {
						if (id[1] >= 0 && id[1] < this.getHeight()) {
							sinogram.addAtIndex(theta, detPixel, (float) (xspacePhantom* InterpolationOperators.interpolateLinear(this, id[0], id[1])));
						}
					}
				}
			}
		...
		for (int beta = 0; beta < numberProj; beta += rotAngle) {
			...
			for (int detPixel = 0; detPixel < numberDetPixel; detPixel++) {
				...
				for (double l = 0; l < dSD; l += xspacePhantom) {
					...
					// Phantom
					// float x = (float) ((float) sourcePos[0]+ l* Math.cos(t *
					// detPixel));
					// float y = (float) ((float) sourcePos[1]+ l* Math.sin(t *
					// detPixel));
		...
		for (int x = 0; x < sinogram.getHeight(); x++) {
			for (int y = 0; y < sinogram.getWidth(); y++) {
				sinogram.addAtIndex(
						x,
						y,
						InterpolationOperators.interpolateLinear(fanogram, dSI* Math.sin(x), x + y));
			}
		}
	...
	public static void main(String[] args) {
		...
		p.createFanogram(180, detectorSpacing, (int) ((int) d / detectorSpacing), d / 2, 1, d / 5);
